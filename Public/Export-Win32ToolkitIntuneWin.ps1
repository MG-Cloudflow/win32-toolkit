...
.DESCRIPTION
    ...
    3. Copies the raw project into Staging\<Template>\<ProjectName>\, EXCLUDING everything that
    ...
    4. Runs Optimize-Win32ToolkitProject against the Staging copy as a safety net — strips any
       surviving non-shipping folder (a locked Intune\ folder aborts packaging), root *.md and
       *.wsb files, the TargetedDocumentationScript, and empty dirs. Projects\ stays untouched.
    5. Runs IntuneWinAppUtil.exe against the Staging copy, outputting to IntuneWin\<Template>\.
