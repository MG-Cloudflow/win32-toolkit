...
| `HyperVBaseVhdx` | — | path | Record of where provisioning placed the golden VHDX. Informational only — never read back by the module. |
...
| `HyperVGuestUser` | — | username | Local-admin account inside the test VM (for PowerShell Direct). |
| `HyperVGuestSecret` | — | DPAPI blob | The guest admin password, encrypted. Never store a plain-text password here. |
